...
public class MapperUtil {
	...
	public static PetDTO convertPetEntityToDto(Pet pet) {
		...
		petDTO.setGender(pet.getGender());
		petDTO.setType(pet.getType());
		List <OwnerDTO> ownerDTO = pet.getOwner().stream().map(owner -> convertOwnerEntityToDtoWithoutPet(owner)).collect(Collectors.toList());
		petDTO.setOwnerDTOList(ownerDTO);
		return petDTO;
	}
	...
	public static Owner convertOwnerDtoToEntity(OwnerDTO ownerDTO) {
		...
		List<Pet> petList = ownerDTO.getPetDTOList().stream().map(petDTO -> convertPetDtoToEntity(petDTO)).collect(Collectors.toList());
