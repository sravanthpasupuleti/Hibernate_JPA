...
public class MapperUtil {
	...
	public static OwnerDTO convertOwnerEntityToDto(Owner owner) {
		...
		List<PetDTO> petList = owner.getPet().stream().map(pet -> convertPetEntityToDtoWithoutOwner(pet))
													  .collect(Collectors.toList());
		// PetDTO petDTO = convertPetEntityToDtoWithoutOwner(owner.getPet());
		ownerDTO.setPetDTOList(petList);
		return ownerDTO;
	}
	...
	public static Owner convertOwnerDtoToEntity(OwnerDTO ownerDTO) {
		...
		List<Pet> petList = ownerDTO.getPetDTOList().stream().map(pet -> convertPetDtoToEntity(pet)).collect(Collectors.toList());
